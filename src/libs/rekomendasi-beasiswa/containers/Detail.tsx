...
import { getRekomendasiBeasiswa } from 'src/stores/rekomendasi-beasiswa/rekomendasiBeasiswaAction'
...
export default function DetailContainer() {
  ...
  const handleGetData = async () => {
    ...
    // @ts-ignore
    await dispatch(getRekomendasiBeasiswa({ id })).then(res => {
  ...
  return (
    <Grid container spacing={2}>
      <Grid item xs={12}>
        <HeaderPage
          title='Detail Rekomendasi Beasiswa'
          isDetail={true}
          action={
            <Box sx={{ display: 'flex', gap: 2 }}>
              <Can I='VERIFICATION' a='REKOMENDASI_BEASISWA'>
                <Button
                  variant='outlined'
                  color='error'
                ...
                <Grid item xs={6}>
                  ...
                  >
                    <Grid container spacing={2}>
                      ...
                      <Grid item xs={8}>
                        <Typography variant='body1' color={'text.secondary'}>
                          {formatString(data?.tipeRekomendasi ?? '-')}
                        </Typography>
                      </Grid>
                    </Grid>
                  </Box>
                      ...
                      <Grid item xs={10}>
                        {data?.dokumenPendukung ? (
                          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                            {data.dokumenPendukung.split(',').map((file: string, idx: number) => {
                              const fileName = file.trim()

                              return fileName ? (
                                <Link key={idx} href={fileName} target='_blank' style={{ textDecoration: 'none' }}>
                                  <Typography variant='body1' sx={{ color: 'blue' }}>
                                    {getFileNamefromURL(fileName) || fileName}
                                  </Typography>
                                </Link>
                              ) : null
                            })}
