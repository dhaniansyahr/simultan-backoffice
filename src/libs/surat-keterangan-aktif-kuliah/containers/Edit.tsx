...
const EditContainer = () => {
  ...
  const onSubmit = handleSubmit(async value => {
    ...
    // @ts-ignore
    await dispatch(updateCollegeCertificate({ data: body, id })).then((res: any) => {
      if (res?.meta?.requestStatus !== 'fulfilled') {
        setIsLoading(false)
        toast.dismiss()
  ...
  const handleGetData = async () => {
    ...
    // @ts-ignore
    await dispatch(getCollegeCertificate({ id })).then((res: any) => {
      if (res?.meta?.requestStatus !== 'fulfilled') {
        setIsLoadData(false)
        toast.dismiss()
        toast.error(res?.payload?.response?.data?.errors?.[0]?.message ?? res?.payload?.response?.data?.message)
  ...
  return (
    <>
      <Card>
        <HeaderPage title='Edit Pengajuan Surat Keterangan Aktif Kuliah' isDetail={true} />
                  ...
                  <Grid item xs={6}>
                    ...
                    >
                      ...
                      <Typography variant='body2'>
                        Bagi Mahasiswa yang orangtua-nya sudah pensiun , surat KP4 diganti dengan foto kopi SK surat
                        pensiun orangtua yang bersangkutan. Bagi Mahasiswa yang orangtua-nya pegawai swasta , surat KP4
